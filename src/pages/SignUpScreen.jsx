...
const SignUpScreen = ({ onSignUpSuccess, onSwitchToSignIn, onBack }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const { user, session, error, needsEmailVerification } = await authService.signUpWithEmail(
            formData.email,
            formData.password
        );
        ...
        if (needsEmailVerification) {
            ...
            // User is automatically signed in (if email verification is disabled in Supabase)
            const userData = {
                id: user.id,
                email: user.email,
